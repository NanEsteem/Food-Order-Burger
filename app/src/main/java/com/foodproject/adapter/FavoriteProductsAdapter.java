...
public class FavoriteProductsAdapter extends RecyclerView.Adapter<FavoriteProductsAdapter.ItemHolder>{
    ...
    public FavoriteProductsAdapter(Context context){
        ...
        try {
            ...
            throw new ClassCastException("Aktywność");
        }

        String[] favoriteFoodNames = {"Klasyczne", "Serowo", "Cheddar",
                "Stek", "Podwójny stek", "Dużo sera", "X Bekon", "Podwójny Bekon", "Hot Dog", "Premium"};

        String[] favoriteFoodPlaces = {"Mc'Donald", "Pod Papugami",
                "Starbucks", "Subway", "Kuźnia Smaku", "Kurna chata", "Charlotte", "Setka", "Starbucks", "Dunkin Donuts"};

        String[] favoriteFoodPrices = {"19,99", "26,00", "15,00",
                "32,00", "20,00", "22,50", "17,99", "23,00", "32,00", "34,00"};

        for (int i = 0; i < 10; i++){
            Product prod = new Product(favoriteFoodNames[i], "Opis " + (i + 1),
                    favoriteFoodPlaces[i],  favoriteFoodPrices[i] + "zł");
            products.add(prod);
        }
    }
