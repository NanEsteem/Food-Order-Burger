...
public class ExtrasAdapter extends RecyclerView.Adapter<ExtrasAdapter.ItemHolder>{
    ...
    private String[] extrasNames = {"Tuńczyk, Łosoś, Wasabi, Ośmiornica, Warzywa, Makaron"};
    ...
    public ExtrasAdapter(Context context){
        ...
        String[] extrasNames = {"Extra ser", "Bekon", "Ser cheddar", "Hamburger",
                "Sos BBQ", "Sałata"};
        String[] extrasValues = {"2", "4", "2", "9", "4", "3"};

        for (int i = 0; i < 6; i++){
            Extra extra = new Extra((i + 1), extrasNames[i], extrasValues[i] + ",00" + "zł");
            mExtras.add(extra);
        }
    }
